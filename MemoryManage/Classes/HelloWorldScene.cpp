...
void HelloWorld::OnClickMenu(Ref* pSender)
{
    ...
    if (tag==200) {
        ...
        for (int i = 0; i<100; i++) {
            ...
            log("%s",e.getFirst()->getCString());
            
            __String * s = (__String *)e.getEmails()->getLastObject();
            log("%s",s->getCString());
        }
        ...
        for (int i = 0; i<100; i++) {
            ...
            e.setFirst(__String::create("zhang"));
            log("%s",e.getFirst()->getCString());
